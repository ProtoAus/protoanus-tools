...
#include <light/model_import.hh> // protoanus-tools: external mesh occluders
...
void Embree_TraceInit(const mbsp_t *bsp)
{
    ...
    // protoanus-tools: external mesh occluders (Source -StaticPropPolys analogue). Added as
    // unconditional casters, exactly like the skip-brush windings above (no intersect filter
    // attached => they always occlude), so `_light_mesh` props throw accurate mesh shadows.
    std::vector<polylib::winding3f_t> meshwindings;
    lightmesh::GatherMeshOccluders(meshwindings);
    CreateGeometryFromWindings(device, scene, meshwindings);
