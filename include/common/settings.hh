...
// thrown after an argument PARSE ERROR (unknown/invalid option, or the wrong number of
// operands). The command-line tools catch this and exit with a NON-ZERO status so build
// front-ends (TrenchBroom, scripts) detect the failure, instead of the old behaviour where
// a bad flag printed the help text and returned 0, looking like success and silently
// producing no output. The help + error text has already been printed at throw time.
struct quit_after_error_exception : public std::exception
{
};
