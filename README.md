...
A fork of **[ericw-tools](https://github.com/ericw-tools/ericw-tools)** — the modern Quake/Half-Life/Source
map compiler suite (`qbsp` / `vis` / `light`, Embree 4) by Eric Wasylishen, itself descended from Tyrann's
tools and id Software's originals. Maintained by **[ProtoAus](https://github.com/ProtoAus)**
(same author as the [slopbroom](https://github.com/ProtoAus) TrenchBroom fork), primarily to sharpen the
`light` compiler and its ergonomics for the FTEQW workflow.

Repo: <https://github.com/ProtoAus/protoanus-tools>

## What this fork adds over upstream

**Implemented**
- **Non-zero exit code on argument errors.** Upstream `qbsp`/`vis`/`light` printed their help text and
  returned **0** on an unknown/invalid option or wrong operand count — so build front-ends (TrenchBroom
  compile profiles, scripts) saw "success" and silently shipped a map with *no lighting*. These tools now
...
## ericw-tools (upstream README follows)
